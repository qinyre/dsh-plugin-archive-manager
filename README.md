...
为 dsh 提供两项能力：会话归档的管理（浏览、还原、可配置的自动归档），以及对话区左侧一条用于轮次导航的刻度尺。两项功能相互独立，可单独使用。

## 对话刻度尺

对话区左缘常驻一条纵向刻度尺，每轮对话（一条用户消息）对应一个刻度。刻度间距固定，不随会话长度变化：刻度较少时整列垂直居中，新增刻度自中部向两端延伸；超出可视高度后，刻度尺出现自带的细滚动条（支持滚轮与滑块拖动），不干预对话本身的滚动。
...
侧边栏底部提供「已归档会话」入口，打开管理面板：已归档会话按工作区分组展示，支持搜索过滤与批量选择，可逐条或批量取消归档。还原经由工作区注册表自身的写入路径完成，写入后所有已连接页面实时恢复侧边栏条目，无需重启。列表中的标题、最近输入与轮次数均取自会话日志本体。

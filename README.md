...
对话区左缘常驻一条纵向刻度尺，每轮对话（一条用户消息）对应一个刻度。刻度间距固定，不随会话长度变化：刻度较少时整列垂直居中，新增刻度自中部向两端延伸；超出可视高度后，刻度尺出现自带的细滚动条（支持滚轮与滑块拖动），不干预对话本身的滚动。打开会话时刻度尺只索引首屏消息，其余历史在后台利用空闲帧逐页补全，长会话切换与浏览不受其拖累。
